...
## Изостазия и батиметрия

- **Turcotte & Schubert 2002** — *Geodynamics*, 2nd ed. Cambridge. [§2.2, §2.6 water loading, §3.13 flexure]
- **Watts 2001** — *Isostasy and Flexure of the Lithosphere*. Cambridge. [Table 5.1 Te values]
- **Parsons & Sclater 1977** — Ocean floor bathymetry and heat flow. [half-space cooling model]
- **Stein & Stein 1992** — Model for the global variation in oceanic depth and heat flow (GDH1). *JGR*.
...
- **Tucker & Bras 2000** — A stochastic approach to modeling the role of rainfall variability. *WRR*.
- **Pelletier 2010** — Minimizing the grid-resolution dependence of flow-routing algorithms. *Geomorphology*.
...
- **Blackman & Vigna 2021** — Scrambled linear pseudorandom number generators. *ACM TOMS* 47(4).
- **Shimrat 1962** — Algorithm 112: Position of point relative to polygon. *CACM*.
- **Steele et al. 2014** — Fast splittable pseudorandom number generators. *OOPSLA*.
